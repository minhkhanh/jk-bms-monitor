/// Compute a simple wrapping-add checksum over `data`.
...
pub fn ascii_to_string(ascii: impl AsRef<[u8]>) -> Result<String, std::str::Utf8Error> {
...
pub fn ascii_to_string_safe(field: &str, ascii: impl AsRef<[u8]>) -> String {
    ...
    ascii_to_string(ascii)
        .map_err(|error| {
            eprintln!("Bad string at '{field}': {error:?}");
        })
        .unwrap_or_default()
}
...
pub fn u32le_to_count(raw: &[u8; 4]) -> u32 {

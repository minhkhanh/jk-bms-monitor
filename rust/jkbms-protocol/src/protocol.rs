...
use crate::types::{CellData, DeviceInfo};
use crate::utils::*;
...
/// Service UUID for JK BMS (0xFFE0)
pub const SERVICE_UUID: &str = "0000ffe0-0000-1000-8000-00805f9b34fb";
/// Characteristic UUID for JK BMS (0xFFE1)
pub const CHARACTERISTIC_UUID: &str = "0000ffe1-0000-1000-8000-00805f9b34fb";

/// Command code for requesting cell data
pub const CMD_CELL_DATA: u8 = 0x96;
/// Command code for requesting device info
pub const CMD_DEVICE_INFO: u8 = 0x97;

/// Record type for cell data response
pub const RECORD_TYPE_CELL_DATA: u8 = 0x02;
/// Record type for device info response
pub const RECORD_TYPE_DEVICE_INFO: u8 = 0x03;
...
impl TryFrom<&'_ RawDeviceInfo> for DeviceInfo {
    ...
    fn try_from(raw: &'_ RawDeviceInfo) -> Result<Self> {
        if raw.record.record_type != RECORD_TYPE_DEVICE_INFO {
...
impl TryFrom<&'_ RawCellData> for CellData {
    ...
    fn try_from(raw: &'_ RawCellData) -> Result<Self> {
        if raw.record.record_type != RECORD_TYPE_CELL_DATA {
            return Err(Error::BadRecordType);
        }
        Ok(Self {
...
mod tests {
    ...
    mod response_parse {
        ...
        fn device_info() {
            let raw = [
                ,
            ];

            let info = DeviceInfo::try_from(raw.as_slice()).unwrap();
        ...
        fn cell_data() {
            ...
            let info = CellData::try_from(raw.as_slice()).unwrap();
